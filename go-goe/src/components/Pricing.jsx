...
const PricingCard = ({ title, price, features, isHighlighted }) => {
  return (
    <div className={`border rounded-lg p-6 shadow-lg ${isHighlighted ? 'border-red-700' : 'border-gray-300'} bg-white dark:bg-gray-800 dark:border-gray-700`}>
      ...
      <button className={`w-full py-2 px-4 rounded-md text-white ${isHighlighted ? 'bg-red-700 hover:bg-red-600' : 'bg-gray-900 hover:bg-gray-800'} dark:bg-gray-600 dark:hover:bg-gray-500`}>
        Choose Plan
      </button>
    </div>
  );
